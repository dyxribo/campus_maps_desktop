...
tween.updateTo({x:300, y:0}, false);
		...
		public function updateTo(vars:Object, resetDuration:Boolean=false):* {
			...
			if (_initted) {
				if (resetDuration) {
					...
					if (_time / _duration > 0.998) { //if the tween has finished (or come extremely close to finishing), we just need to rewind it to 0 and then render it again at the end which forces it to re-initialize (parsing the new vars). We allow tweens that are close to finishing (but haven't quite finished) to work this way too because otherwise, the values are so small when determining where to project the starting values that binary net.blaxstar.math issues creep in and can make the tween appear to render incorrectly when run backwards.
		...
		override public function render(time:Number, suppressEvents:Boolean=false, force:Boolean=false):void {
			...
			} else if (time < 0.0000001) { //to work around occasional floating point net.blaxstar.math artifacts, round super small values to 0.
...
myAnimation.yoyo( true ); //sets yoyo to true
		...
		public static function globalTimeScale(value:Number=NaN):Number {
			...
			value = value || 0.0001; //can't allow zero because it'll throw the net.blaxstar.math off
