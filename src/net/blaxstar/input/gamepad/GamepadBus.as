...
/**
	...
	public class GamepadBus {
		...
		public static var readyControllers:Vector.<Gamepad>   = new Vector.<Gamepad>;
		public static var removedControllers:Vector.<Gamepad> = new Vector.<Gamepad>;
		public static var isInitialized:Boolean = false;
		private static var gameInput:GameInput;
		private static var customListener:Function;
		...
		public static function initialize(stage:DisplayObject, gamepadConnectListener:Function = null):void {
			...
			if (gamepadConnectListener != null) customListener = gamepadConnectListener;
		 ...
		 */
		public static function getReadyController():Gamepad {
			var readyController:Gamepad = readyControllers.shift();
		 ...
		 */
		public static function getRemovedController():Gamepad {
			var removedController:Gamepad = removedControllers.shift();
		...
		private static function detach(device:GameInputDevice):void {
			...
			var detachedController:Gamepad = findAndRemoveDevice(controllers, device) || findAndRemoveDevice(readyControllers, device);
		 ...
		 */
		private static function findAndRemoveDevice(source:Vector.<Gamepad>, target:GameInputDevice):Gamepad {
		...
		/**
		 * Callback for keyboard events that catches the back and escape keys such that stupid bindings don't exit the application.
		 *
